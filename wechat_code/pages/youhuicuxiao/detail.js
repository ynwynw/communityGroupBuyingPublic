...
tablename: 'youhuicuxiao',
userid:userInfo.id,
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.huodongmingcheng,
...
const {
data
} = await detail("youhuicuxiao",id)
this.setData({
payAuth:utils.isAuthFront('youhuicuxiao','支付')
})
...
const resUpdate = await update('youhuicuxiao', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','youhuicuxiao');
...
async reply() {
const detailList = this.data.detailList
const res = await update("youhuicuxiao", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
