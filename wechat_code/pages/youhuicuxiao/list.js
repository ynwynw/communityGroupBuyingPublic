...
if(this.data.huodongmingcheng){
    searchForm['huodongmingcheng'] = '%' + this.data.huodongmingcheng + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`youhuicuxiao`, searchForm);
} else {
res = await list(`youhuicuxiao`, searchForm);
}
...
let targetName="huodongmingcheng"
...
wx.navigateTo({
url: `/pages/youhuicuxiao/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("youhuicuxiao",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/youhuicuxiao/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
console.log('11', displayReporter.showReferpagepath)
if (displayReporter.showReferpagepath.includes('center')) {
    return true
} else {
    return  false
}
...
let isAuthObj={}
if(this.checkCurPath()){
isAuthObj['addAuth']   = utils.isAuth("youhuicuxiao", "新增")
isAuthObj['delAuth']  = utils.isAuth("youhuicuxiao", "删除")
isAuthObj['editAuth']   =utils.isAuth("youhuicuxiao", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("youhuicuxiao", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("youhuicuxiao", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("youhuicuxiao", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    ...
    if(this.data.userid){
        resList = await page("youhuicuxiao",obj)
    }else{
         resList= await list("youhuicuxiao",obj)
    }
