...
wx.setStorageSync('infomation', {remark:this.data.remark,userid:this.data.userInfo.id,
...
consignee:this.data.address[0]?.name,
        ...
        wx.navigateTo({
          url: '/pages/keyboardNumber/keyboardNumber',
        })
    },
    //正常支付
     payClick() {
...
//收货地址
    ...
    if(infodata.jf){
        //打印有没有被减去
        this.setData({
            'userInfo.jf': parseInt(this.data.userInfo.jf) + parseInt(item.price * item.buynumber)
        })
...
//收货地址
        ...
        consignee:this.data.types==1?'': this.data.address[0].name,
                ...
                //新增订单
                const addRes = await add("orders", data)
                this.setData({
                    'userInfo.jf': parseInt(this.data.userInfo.jf) - parseInt(item.price * item.buynumber)
                })
 //如果商品有库存 减去商品库存
